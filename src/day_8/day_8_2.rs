    ...
    for x in antehna_index.0..map.len() {
        for y in 0..map[0].len() {
            if map[x][y] == antehna && (x, y) != antehna_index {
                antinode_map[antehna_index.0][antehna_index.1] = '#';
                ...
                while freq_first.0 < (map.len() as i32)
                    && freq_first.0 >= 0
                    && freq_first.1 < (map[0].len() as i32)
                    && freq_first.1 >= 0
                {
                    antinode_map[freq_first.0 as usize][freq_first.1 as usize] = '#';
                    if antehna_index.1 > y {
                        freq_first = (
                            (freq_first.0 as i32 - diff.0 as i32) as i32,
                            (freq_first.1 as i32 + diff.1 as i32) as i32,
                        );
                    } else {
                        freq_first = (
                            (freq_first.0 as i32 - diff.0 as i32) as i32,
                ...
                while freq_second.0 < (map.len() as i32)
                    && freq_second.0 >= 0
                    && freq_second.1 < (map[0].len() as i32)
                    && freq_second.1 >= 0
                {
                    antinode_map[freq_second.0 as usize][freq_second.1 as usize] = '#';
                    if antehna_index.1 > y {
                        freq_second = (
                            (freq_second.0 as i32 + diff.0 as i32) as i32,
                            (freq_second.1 as i32 - diff.1 as i32) as i32,
                        );
                    } else {
                        freq_second = (
                            (freq_second.0 as i32 + diff.0 as i32) as i32,
